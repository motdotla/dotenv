...
function _configVault (options) {
  ...
  DotenvModule.apply(parsed, process.env, options)
...
function apply (source, target, options = {}) {
  const debug = Boolean(options && options.debug)
  const override = Boolean(options && options.override)

  if (typeof source !== 'object') {
    throw new Error('INVALID_SOURCE_OBJECT:: Please check the source object being passed to apply')
  }
