...
// Match the whitespace characters recognized by JavaScript's \\s. The fast
// parser cannot use a regular expression in its hot path, but it still needs
// to accept the same input as the classic parser.
...
function parseFast (src) {
  ...
  while (i < len) {
    ...
    if (i < len && isWhitespace(c)) return parseRegex(str)
    if (i >= len) break
    ...
    if (c === 101 /* e */ && i + 6 < len &&
      ...
      if (nc === 32 || nc === 9 || nc === 10 || nc === 65279) {
        i += 7
        while (i < len && ((c = str.charCodeAt(i)) === 32 || c === 9 || c === 10 || c === 65279)) i++
        if (i < len && isWhitespace(c)) return parseRegex(str)
    ...
    // Skip whitespace after the separator. A newline only belongs to the value
    // when the next non-whitespace character starts a quoted value; otherwise
    // it terminates an empty value and begins the next assignment.
    while (i < len && ((c = str.charCodeAt(i)) === 32 || c === 9 || c === 65279)) i++
    if (i < len && c !== 10 && isWhitespace(c)) return parseRegex(str)
    if (i < len && str.charCodeAt(i) === 10) {
      let j = i
      while (j < len && isWhitespace(str.charCodeAt(j))) j++
      const next = j < len ? str.charCodeAt(j) : 0
      if (next === 39 || next === 34 || next === 96) {
        let k = j + 1
        while (k < len) {
          const cc = str.charCodeAt(k)
          if (cc === 92 && k + 1 < len) {
            const nc = str.charCodeAt(k + 1)
            if (nc === next || nc === 92) {
              k += 2
              continue
            }
        ...
        if (k < len) {
          let tail = k + 1
          let tc = tail < len ? str.charCodeAt(tail) : 0
          while (tail < len && isWhitespace(tc) && tc !== 10) {
            tail++
            tc = tail < len ? str.charCodeAt(tail) : 0
          }
          if (tail >= len || tc === 10 || tc === 35) i = j
        }
      }
    }
    ...
    if (c === 39 /* ' */ || c === 34 /* " */ || c === 96 /* ` */) {
      ...
      if (j >= len) {
        ...
        if (i < len && str.charCodeAt(i) === 35) {
          while (i < len && str.charCodeAt(i) !== 10) i++
        } else if (i < len && str.charCodeAt(i) !== 10) {
          // A closing quote followed by non-comment text is an unquoted value
          // in the classic parser, including the quote characters themselves.
          let end = str.indexOf('\n', i)
          if (end === -1) end = len
          const hash = str.indexOf('#', i)
          if (hash !== -1 && hash < end) end = hash
          value = str.slice(quoteStart, end).trim()
          i = end
        }
      }
    } else {
