...
// Parse src into an Object — character scanner for common input, with the
// classic parser handling uncommon whitespace and ambiguous quoted values.
...
function parseFast (src) {
  ...
  // Match JavaScript whitespace around keys, separators and export prefixes.
  if (/[\v\f\u00A0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF]/.test(str)) {
  ...
  while (i < len) {
    ...
    // skip whitespace / blank lines (other whitespace is handled above)
    ...
    if (c === 10) {
      // Quotes can start on a later line; ordinary empty values must not
      // consume the next key, so only look ahead before choosing a parser.
      let j = i + 1
      while (j < len && (str.charCodeAt(j) === 32 || str.charCodeAt(j) === 9 || str.charCodeAt(j) === 10)) j++
      const next = str.charCodeAt(j)
      if (next === 39 || next === 34 || next === 96) return parseRegex(str)
    }
    ...
    if (c === 39 /* ' */ || c === 34 /* " */ || c === 96 /* ` */) {
      ...
      if (j >= len) {
        // Even an unterminated double quote triggers escape expansion.
        return parseRegex(str)
      } else {
        // Trailing text may force regex backtracking or unquoted quote
        // stripping. Defer to the classic parser instead of losing text.
        let end = j + 1
        while (end < len && (str.charCodeAt(end) === 32 || str.charCodeAt(end) === 9)) end++
        if (end < len && str.charCodeAt(end) !== 10 && str.charCodeAt(end) !== 35) return parseRegex(str)
