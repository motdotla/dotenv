...
function parseFast (src) {
  ...
  while (i < len) {
    ...
    if (stop === 61 /* = */) {
      ...
      // the next assignment when this key has no separator. The same holds
      // for the lookahead after 'export', so resume from its own line.
...
function configDotenv (options) {
  ...
  // Read quiet after loading .env, while preserving explicit options and
  // startup environment settings (including false).
  const quiet = parseBoolean(Object.prototype.hasOwnProperty.call(options, 'quiet')
    ? options.quiet
    : optionsFromEnv(processEnv).quiet)
