...
const os = require('os')
const { parseQuotedAfterNewline } = require('./quoted-after-newline.js')
...
function parseFast (src) {
  ...
  while (i < len) {
    ...
    if (stop === 61 /* = */) {
      i++
      afterSeparator = '='
    } else if (stop === 58 /* : */ && !sawSpaceBeforeSeparator && i + 1 < len && (str.charCodeAt(i + 1) === 32 || str.charCodeAt(i + 1) === 9)) {
      i++
      afterSeparator = ': '
    } else {
      // invalid line — skip
    ...
    if (afterSeparator === ': ') {
      ...
      if (i > 0 && (str.charCodeAt(i - 1) === 32 || str.charCodeAt(i - 1) === 9)) i--
    }
