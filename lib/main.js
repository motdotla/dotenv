  ...
  while (i < len) {
    ...
    if (quote === 39 || quote === 34 || quote === 96) {
      ...
      while (j < len) {
        ...
        // A backslash only escapes a matching quote. Backslash pairs are
        // preserved, so the last backslash in a run can still escape it.
        const escaped = cc === 92 && str.charCodeAt(j + 1) === quote
        if (escaped) j++
        if (cc === quote || escaped) {
          let end = j + 1
