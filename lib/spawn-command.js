...
// one layer of carets (batch files parse their forwarded arguments again).
...
function spawnCommand (command, args, options) {
  ...
  // Both ordinary batch files and npm shims reparse arguments forwarded by %*.
  // Protect that second parse regardless of where the batch file is installed.
  const batchFile = /\.(?:bat|cmd)$/i.test(file || command)
  const tokens = [protectShellToken(path.normalize(file || command))]
  for (const argument of args) {
    tokens.push(protectShellToken(quoteWindowsArgument(argument), batchFile ? 2 : 1))
  }
