...
2. Import `dotenv/config` instead of `dotenv` (_Note: you do not need to call `dotenv.config()` and must pass options via the command line or environment variables with this approach_). For Node 12, which mandates file extensions for `import`, use `import dotenv/config.js`.
